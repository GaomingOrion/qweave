...
散的部分做好：因子表达式、批量计算、标签、评估和报告。在同一份 480 万行合成
面板上，qweave 比 Qlib Alpha158DL 快 23.85×，端到端比 KunQuant 的 JIT C++ 路径
快 2.56×（[实测数据与口径](benchmark.md)）。
...
定性边界之外，[性能与基准测试](benchmark.md)发布了与 Qlib Alpha158DL、KunQuant
在同一合成面板上的实测对比数字，含机器配置、commit 和完整命令。

## 不适合什么

- 需要开箱即用的数据下载、模型训练模板、组合优化和回测实验管理：选 Qlib。
- 核心诉求是把固定表达式 bundle 编译成极致优化的 C++ 代码：选 KunQuant。
- 只做一次性小样本 notebook 分析：pandas/Alphalens 风格工具可能更轻。
